...
namespace Xunit
{
    ...
    public class FactAttribute : Attribute
    {
        ...
        {
            if (Skip != null)
                yield return new SkipCommand(method, DisplayName, Skip);
            else
                foreach (ITestCommand command in EnumerateTestCommands(method))
                {
                    ITestCommand result = command;

                    if (Timeout > 0)
                        result = new TimeoutCommand(result, Timeout, method);
