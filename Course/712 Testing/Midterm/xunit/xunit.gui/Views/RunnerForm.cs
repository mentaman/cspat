...
namespace Xunit.Gui
{
    public partial class RunnerForm : Form, ITestMethodRunnerCallback
    {
        ...
        bool currentProjectIsDirty = false;
        string windowTitle;
        ...
        public RunnerForm()
        {
            ...
            {
                if (!DesignMode)
                    using (var displayKey = Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\xUnit.net\Display"))
                        if (displayKey != null)
                        {
                            Size = new Size((int)displayKey.GetValue("Width"), (int)displayKey.GetValue("Height"));
                            Location = new Point((int)displayKey.GetValue("X"), (int)displayKey.GetValue("Y"));
                            horizontalSplitter.SplitterDistance = (int)displayKey.GetValue("Splitter");
                            verticalSplitter.SplitterDistance = (int)displayKey.GetValue("Splitter2");

                            if ((int)displayKey.GetValue("Maximized") == 1)
                                WindowState = FormWindowState.Maximized;
                        }
            }
            catch { }  // Don't want to throw if the user mucks up the registry
        }
        ...
        void LoadProject(string filename)
        {
            ...
            try
            {
                ...
                currentProject = Path.GetFullPath(filename);
        ...
        void UnloadAssemblies(IEnumerable assemblyListViewItems)
        {
            foreach (ListViewItem assemblyListViewItem in assemblyListViewItems)
            {
                ...
                if (toRemove != null)
                    project.Assemblies.Remove(toRemove);
            }
        ...
        void UpdateRunState()
        {
            string title = currentProject == null ? "Untitled" : Path.GetFileNameWithoutExtension(currentProject);
            if (currentProjectIsDirty)
                title += " *";
            ...
            menuProjectClose.Enabled = !isRunning && currentProject != null;
            menuProjectSave.Enabled = !isRunning && currentProject != null && currentProjectIsDirty;
        ...
        void RunnerForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            using (var xunitKey = Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\xUnit.net"))
            {
                try
                {
                    xunitKey.DeleteSubKeyTree("Display");
                }
                catch (ArgumentException) { }    // Stupid APIs that throw when things don't exist when you want them to not exists

                using (var displayKey = xunitKey.CreateSubKey("Display"))
                {
                    bool normalWindowState = WindowState == FormWindowState.Normal;
                    Point location = normalWindowState ? Location : RestoreBounds.Location;
                    Size size = normalWindowState ? Size : RestoreBounds.Size;

                    displayKey.SetValue("Maximized", WindowState == FormWindowState.Maximized ? 1 : 0);
                    displayKey.SetValue("X", location.X);
                    displayKey.SetValue("Y", location.Y);
                    displayKey.SetValue("Width", size.Width);
                    displayKey.SetValue("Height", size.Height);
                    displayKey.SetValue("Splitter", horizontalSplitter.SplitterDistance);
                    displayKey.SetValue("Splitter2", verticalSplitter.SplitterDistance);
                }
            }
        }
