...
\begin{enumerate}
  ...
  Init($\neg$loaded(b) $\wedge$ $\neg$loaded(nil) $\wedge$ loaded(waste) $\wedge$ coffee(b) $\wedge$ coffee(i)) \\
...
\end{figure}
		...
		\item (5 pts.) Mufasa is a lion, which is a kind of mammal. Therefore, mufasa is a mammal. Add a rule to your prolog database that generalizes this to all animals. Run a sample query to demonstrate this rule and show the output.\\
		
		Rules: \\
		value(Node, isa, Value):-\\
        \hspace*{6ex} edge(Node, isa, Node1),\\
    	\hspace*{6ex} edge(Node1, ako, Value).\\
		
		Query: value(mufasa,isa,X).\\
		Result: X = lion; X = mammals ; \\
		
		Query: value(velocitusIncalculii, isa, X).\\
		Result: X = roadrunner; X = birds ;\\
