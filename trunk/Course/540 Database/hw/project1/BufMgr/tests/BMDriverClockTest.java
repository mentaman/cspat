...
public class BMDriverClockTest extends TestDriver implements GlobalConst {
	...
	public boolean test1() {
		...
		for (pid.pid = firstPid.pid, lastPid.pid = pid.pid + numPages; status == OK
			...
			if (status == OK) {
				...
				try {
					...
					System.out.println("setting data: " + data + " for page "
							+ pid);

					data = Convert.getIntValue(0, pg.getpage());
					System.out.println("checking data: " + data + " for page "
							+ pid);
				} catch (IOException e) {
		...
		for (pid.pid = firstPid.pid; status == OK && pid.pid < lastPid.pid; pid.pid = pid.pid + 1) {
			...
			try {
				System.out.println("try to read page " + pid);
	...
	public boolean test2() {
		...
		if (status == OK) {
			try {
				...
				status = checkException(e,
						"exceptions.BufferPoolExceededException");
				if (status == FAIL) {
		...
		if (status == OK) {
			...
			try {
				...
				status = checkException(e,
						"exceptions.HashEntryNotFoundException");
	...
	public boolean test3() {
		...
		for (index = 0; status == OK && index < numPages; ++index) {
			...
			if (status == OK) {
				...
				// coincidence.
				int data = pid.pid + 99999;
				System.out.println("setting data: " + data + " to page " + pid);
				try {
					Convert.setIntValue(data, 0, pg.getpage());
					int value = Convert.getIntValue(0, pg.getpage());
					System.out.println("-------------------------------");
					System.out.println("after setting, page " + pid + "has data: " + value);
					System.out.println("-------------------------------");
				} catch (IOException e) {
					System.err.print("*** Convert value failed\n");
					status = FAIL;
